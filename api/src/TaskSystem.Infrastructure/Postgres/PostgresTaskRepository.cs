...
public sealed class PostgresTaskRepository(ICoreDbConnectionFactory factory) : ITaskRepository
{
        ...
        int? assigneeId)
    {
        ...
        try
        {
            ...
            int? projectExists = await connection.QueryFirstOrDefaultAsync<int?>(lockProjectSql, new { ProjectId = projectId }, transaction);
            ...
            int currentMax = await connection.ExecuteScalarAsync<int>(nextNumberSql, new { ProjectId = projectId }, transaction);

            int nextNumber = currentMax + 1;

            const string insertSql = "INSERT INTO tasks (project_id, number, title, description, status, priority, assignee_id, created_at) " +
    ...
    public async Task<IReadOnlyList<TaskItem>> GetByProjectAsync(int projectId)
    {
        ...
        IEnumerable<TaskItem> result = await connection.QueryAsync<TaskItem>(sql, new { ProjectId = projectId });
