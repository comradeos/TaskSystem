...
public class PostgresProjectRepository(ICoreDbConnectionFactory factory) : IProjectRepository
{
    ...
    public async Task<IReadOnlyList<Project>> GetPageAsync(int page, int size, CancellationToken ct = default)
    {
        ...
        IEnumerable<Project> result = await connection.QueryAsync<Project>(sql, new
        {
            Size = size,
            Offset = offset
        });
